...
%la trajectoire définie par les points contenus dans [X Y Z] et le plan de normale x positionné à l'abscisse X_plan
...
B=max(A);
Y_Print = ((Y(B+1)-Y(B))/(X(B+1)-X(B)))*(X_plan-X(B))+Y(B); %Calcul via l'équation de la tangente au point le plus proche du plan
